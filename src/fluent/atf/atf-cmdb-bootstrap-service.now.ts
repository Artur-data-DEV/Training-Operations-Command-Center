    ...
    (atf) => {
        atf.server.runServerSideScript({
            ...
            script: `
                ...
                gs.assertTrue(result && result.success === true, 'Bootstrap should succeed.');
                gs.assertTrue(result.room_sys_id === roomId, 'Bootstrap should target created room.');
                gs.assertTrue(result.ci_created === 3, 'Expected ci_created=3, got: ' + result.ci_created);
                gs.assertTrue(result.resource_created === 3, 'Expected resource_created=3, got: ' + result.resource_created);
                gs.assertTrue(result.resources_total === 3, 'Expected resources_total=3, got: ' + result.resources_total);
                ...
                while (gr.next()) {
                    resourceCount++;
                    gs.assertTrue(!gs.nil(gr.getValue('ci_reference')), 'Resource must keep ci_reference.');
                }

                gs.assertTrue(resourceCount === 3, 'Expected 3 room_resource records, got: ' + resourceCount);
            `,
        })
    }
)
    ...
    (atf) => {
        atf.server.runServerSideScript({
            ...
            script: `
                ...
                gs.assertTrue(first.success === true, 'First bootstrap run should succeed.');
                gs.assertTrue(second.success === true, 'Second bootstrap run should succeed.');
                gs.assertTrue(first.ci_created === 3, 'First run should create 3 CIs.');
                gs.assertTrue(first.resource_created === 3, 'First run should create 3 resources.');
                gs.assertTrue(second.ci_created === 0, 'Second run must not create new CIs.');
                gs.assertTrue(second.resource_created === 0, 'Second run must not create new resources.');
                gs.assertTrue(second.ci_existing >= 3, 'Second run should reuse existing CIs.');
                ...
                gs.assertTrue(resourceCount === 3, 'Expected exactly 3 room resources after two runs, got: ' + resourceCount);
                gs.assertTrue(Object.keys(resourceIds).length === 3, 'Duplicate room resource rows detected.');
            `,
        })
    }
)
